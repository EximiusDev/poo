	...
	for(int j=0;j< cant_bloq_plat_y;j++) { 
		for(int i=0;i<this->Get_cant_bloq_plat(j);i++) {
			...
			case 2:
				...
				break;
			case 3:
				Plat3[i].Update( Velocity.x );
				break;
			case 4:
				Plat4[i].Update( Velocity.x );
				break;
			case 5:
				Plat5[i].Update( Velocity.x );
				break;
			case 6:
				Plat6[i].Update( Velocity.x );
				break;
			case 7:
				Plat7[i].Update( Velocity.x );
				break;
			case 8:
				Plat8[i].Update( Velocity.x );
				break;
			default:
				//Plat1[i].Update( Velocity.x); /// Velocidad (speed = -1* 3)
				cerr<<"ERROR: cantidad de platafortmas incorrectas"<<endl;
