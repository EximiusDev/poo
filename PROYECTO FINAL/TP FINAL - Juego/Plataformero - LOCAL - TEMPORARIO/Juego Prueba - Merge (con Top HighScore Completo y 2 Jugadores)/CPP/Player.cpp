...
Player::Player(bool player_2): Object() {
	scaleG = { 24.f/(resolutionG.x/16.f) ,  24.f/(resolutionG.x/16.f) }; // SCALE =  0.3
	spr.setPosition(resolutionG.x * 0.4f + 100, resolutionG.y * 0.5f);
	spr.setScale(scaleG);
	sumar_score = false;
	
	spr.setColor({255,255,0});
	player_two = player_2;
}

void Player::Update(bool collide_With_floor, bool collide_With_top,bool collide_With_wall_left, bool collide_With_wall_right,float velocity, int & life, Game& playgame) {
	//speedG={0,0};
	speedG={- velocity,0};
	...
	if (jumping_time.getElapsedTime().asMilliseconds() >= 600 || (!Keyboard::isKeyPressed(Keyboard::Up) and (!player_two)) || (!Keyboard::isKeyPressed(Keyboard::W) and (player_two)) || collide_With_top && !collide_With_floor) { //Si paso el tiempo O si no fueron apretadas las tecla de salto en el aire...
	...
	if (!on_Air_Falling && ( (Keyboard::isKeyPressed(Keyboard::Up) and (!player_two)) || (Keyboard::isKeyPressed(Keyboard::W) and (player_two)) ) ) { // Si no esta cayendo y se apreto la tecla de salto...
	...
	if (( (Keyboard::isKeyPressed(Keyboard::Right) and (!player_two)) || (Keyboard::isKeyPressed(Keyboard::D) and (player_two)) )&&r1.left + r1.width < resolutionG.x && !collide_With_wall_right ) {
	...
	if (( (Keyboard::isKeyPressed(Keyboard::Left) and (!player_two)) || (Keyboard::isKeyPressed(Keyboard::A)  and (player_two))) && !collide_With_wall_left && r1.left > 0) {
