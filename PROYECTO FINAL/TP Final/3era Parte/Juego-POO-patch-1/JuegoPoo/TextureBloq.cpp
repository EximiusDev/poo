...
TextureBloq::TextureBloq(vector<vector<TipoBloque>>M_Bloq, int altura_E, int ancho_E, int nivelcero_E, int resolucion_x,int resolucion_y, int x, int y) {
	...
	for(int i=0;i<p_lim_y;i++) { 
		for(int j=0;j<p_lim_x;j++) { 
			Textu[i][j].loadFromFile(M_Bloq[y + i][x + j].NomBloq());
...
void TextureBloq::PosSpr(int a,int b, int x1, int y1){  		/// a es el eje y, b es el eje x
	...
	int pant_medio_x = ((res_x/2) + (16*(12/2))); 	// int xp = ((res_x/2) -(16*(12/2)));
	int pant_medio_y = ((res_y/2) + (16*(8/2)));		//int yp = ((res_y/2) -(16*(8/2)));
	
	//Spri[a+y1][b+x1].setPosition( (b) *(16*4), pto_medio_y + (a) *(16*4)); /// (16*4) = 64 pixeles
	Spri[a][b].setPosition( (b) *(16*4), pant_medio_y + (a) *(16*4)); /// (16*4) = 64 pixeles
}
