...
void Pantalla::Run(string Nom){ ///Con este metodo empiezo a correr el juego
	...
	int tam_bloq = 64; /// tamanio de bloque y del personaje
	
	int xp = ((res_x/2) - tam_bloq/2 ); int yp = ((res_y/2) - tam_bloq/2 );
	...
	*/
	...
	while(w.isOpen()) { /// mientras se renderice la pantalla 
		...
		if(Keyboard::isKeyPressed(Keyboard::Key::Left)){ /// veo si el teclado esta presionado y le paso de parametro el metodo Key de Keyboard
			...
			Fondo1.Pos(-x*1.2,-y*1.2 ); /// cambio la posicion de los distintos fondos
			Fondo2.Pos(-x*1.4,-y*1.4 ); /// cambio la posicion del fondo que va por delante mas que el que va por atras, sucesivamente 
			Fondo3.Pos(-x*1.6,-y*1.6 );
			Fondo4.Pos(-x*1.8,-y*1.8);
			Fondo5.Pos(-x*2,-y*2 ); /// la variacion entre el cambio de posicion de los fondos brinda la sensacion de movimiento y tridimensionalidad
			
			
			xp = ((res_x/2) -(tam_bloq/2)); yp = ((res_y/2) -(tam_bloq/2));
