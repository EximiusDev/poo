...
void Pantalla::Start(){
	...
	Vector2i r1 = Mus.getPosition(wind);
		...
		wind.draw(Men.Salir());
			if(Mus.isButtonPressed(Mus.Left)&&Men.NuevoJuego().getGlobalBounds().contains(r1.x,r1.y)){
			Click= true;
			}
			if(Mus.isButtonPressed(Mus.Left)&&Men.Salir().getGlobalBounds().contains(r1.x,r1.y)){
				wind.close();
			}
		if(Click){
		//if (Keyboard::isKeyPressed(Keyboard::Key::Escape)){wind.close();}
			...
			case 3:
				...
				if(Mus.isButtonPressed(Mus.Left)&&GO.Reiniciar().getGlobalBounds().contains(r1.x,r1.y)){
					Vid.Reinicio();
					Perdio =false;
				}
			}}
