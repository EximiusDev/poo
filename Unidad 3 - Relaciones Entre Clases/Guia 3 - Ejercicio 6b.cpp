...
public:
	...
	bool Pertenece(Punto p){return esIgual(p.y,((m * p.x) + b));} /// anda bien salvo por el redondeo de float (decimales redondeados distintos)
};
...
public:
	...
	string ObtenerEcuacion(){
		...
		bool pertenece(Punto p){return esIgual(a*p.x+b*p.y+c , 0.0);} /// anda con la funcion esIgual
};
